...
def fix_structure(text: str) -> tuple[str, list[str]]:
    ...
    if "content_ref:" not in fm_raw:
        if "akms_schema:" in fm_raw:
            fm_raw = fm_raw.replace("akms_schema:", "content_ref: null\n\nakms_schema:")
        else:
...
def validate_node(text: str) -> list[dict]:
    ...
    eq_refs = re.findall(r"(?:Eq\.|Equation|equation)\s*[\(\[]?\d+[\.\d]*[\)\]]?", body)
    for ref in eq_refs:
...
def process_file(
    ...
    if not validate_only:
        ...
        if verbose and (cstats["span_tags"] or cstats["citation_brackets"]):
            print(
                f"  [clean] {cstats['span_tags']} span tags, "
...
def main():
    ...
    if args.input.is_dir():
        ...
        if not args.input.exists():
            ...
            sys.exit(1)
        ok = process_file(
            args.input, args.output, args.validate_only, args.clean_only, args.verbose
        )
        sys.exit(0 if ok else 1)
